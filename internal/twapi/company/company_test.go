...
func TestSingle(t *testing.T) {
	...
	var companyID int64
	companyIDSetter := twapi.WithIDCallback("id", func(i int64) {
		companyID = i
	})
...
func TestCreate(t *testing.T) {
	...
	tests := []struct {
		...
		create: company.Create{
			Name:        fmt.Sprintf("test%d%d", time.Now().UnixNano(), rand.Intn(100)),
			AddressOne:  twapi.Ref("123 Main St"),
			AddressTwo:  twapi.Ref("Apt. 456"),
			City:        twapi.Ref("Cork"),
			CountryCode: twapi.Ref("IR"),
			EmailOne:    twapi.Ref("test1@company.com"),
			EmailTwo:    twapi.Ref("test2@company.com"),
			EmailThree:  twapi.Ref("test3@company.com"),
			Fax:         twapi.Ref("123-456-7890"),
			Phone:       twapi.Ref("123-456-7890"),
			Profile:     twapi.Ref("This is a test company profile."),
			State:       twapi.Ref("Cork"),
			Website:     twapi.Ref("https://www.example.com"),
			Zip:         twapi.Ref("12345"),
			ManagerID:   &resourceIDs.userID,
			IndustryID:  twapi.Ref(int64(1)), // Web Development Agency,
