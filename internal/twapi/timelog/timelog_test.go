...
func TestSingle(t *testing.T) {
	...
	var timelogID int64
	timelogIDSetter := twapi.WithIDCallback("id", func(i int64) {
		timelogID = i
	})
...
func TestCreate(t *testing.T) {
	...
	tests := []struct {
		...
		create: timelog.Create{
			Description: twapi.Ref("This is a test timelog"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var timelogID int64
			timelogIDSetter := twapi.WithIDCallback("id", func(id int64) {
				timelogID = id
			})
...
func TestUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		create: timelog.Update{
			...
			Date:        twapi.Ref(twapi.Date(time.Now().UTC())),
			Time:        twapi.Ref(twapi.Time(time.Now().UTC())),
			IsUTC:       twapi.Ref(true),
			Hours:       twapi.Ref(int64(2)),
			Minutes:     twapi.Ref(int64(15)),
			Billable:    twapi.Ref(true),
			UserID:      twapi.Ref(resourceIDs.userID),
			TagIDs:      []int64{resourceIDs.tagID},
		},
	}}
