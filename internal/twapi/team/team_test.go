...
import (
	...
	"github.com/rafaeljusto/teamwork-ai/internal/twapi/team"
...
func TestSingle(t *testing.T) {
	...
	var teamID int64
	teamIDSetter := twapi.WithIDCallback("id", func(i int64) {
		teamID = i
	})
	...
	single.ID = twapi.LegacyNumber(teamID)

	if err := engine.Do(ctx, &single); err != nil {
		t.Fatalf("failed to get team: %v", err)
	}
	if single.ID != twapi.LegacyNumber(teamID) {
...
func TestCreate(t *testing.T) {
	...
	tests := []struct {
		...
		create: team.Create{
			Name:         fmt.Sprintf("test%d%d", time.Now().UnixNano(), rand.Intn(100)),
			Handle:       twapi.Ref(fmt.Sprintf("testhandle%d%d", time.Now().UnixNano(), rand.Intn(100))),
			Description:  twapi.Ref("This is a test team."),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var teamID int64
			teamIDSetter := twapi.WithIDCallback("id", func(id int64) {
				teamID = id
			})
