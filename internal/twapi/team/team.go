...
type Team struct {
	...
	ProjectID twapi.LegacyNumber `json:"projectId"`
	Company   *struct {
		ID   twapi.LegacyNumber `json:"id"`
		Name string             `json:"name"`
	} `json:"company"`
	...
	Members []twapi.LegacyRelationship `json:"members"`

	CreatedBy twapi.LegacyNumber      `json:"createdByUserId"`
	CreatedAt time.Time               `json:"dateCreated"`
	UpdatedBy twapi.LegacyNumber      `json:"updatedByUserId"`
...
func (t *Team) PopulateResourceWebLink(server string) {
	...
	t.WebLink = twapi.Ref(fmt.Sprintf("%s/app/teams/%d", server, t.ID))
}
...
type Create struct {
	...
	UserIDs      twapi.LegacyNumericList `json:"userIds,omitempty"`
}
