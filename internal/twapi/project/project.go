...
func (p *Project) PopulateResourceWebLink(server string) {
	...
	p.WebLink = twapi.Ref(fmt.Sprintf("%s/app/projects/%d", server, p.ID))
}
...
type Create struct {
	...
	StartAt     *twapi.LegacyDate `json:"start-date,omitempty"`
	EndAt       *twapi.LegacyDate `json:"end-date,omitempty"`
