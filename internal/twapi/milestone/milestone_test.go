...
func TestSingle(t *testing.T) {
	...
	create := milestone.Create{
		...
		Assignees: twapi.LegacyUserGroups{
	...
	milestoneIDSetter := twapi.WithIDCallback("milestoneId", func(i int64) {
		milestoneID = i
	})
...
func TestCreate(t *testing.T) {
	...
	tests := []struct {
		...
		create: milestone.Create{
			Name:        fmt.Sprintf("test%d%d", time.Now().UnixNano(), rand.Intn(100)),
			Description: twapi.Ref("This is a test milestone"),
			DueDate:     twapi.LegacyDate(time.Now().Add(48 * time.Hour)),
			ProjectID:   resourceIDs.projectID,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var milestoneID int64
			milestoneIDSetter := twapi.WithIDCallback("milestoneId", func(id int64) {
				milestoneID = id
			})
...
func TestUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		create: milestone.Update{
			...
			Description: twapi.Ref("This is a test milestone"),
			DueDate:     twapi.Ref(twapi.LegacyDate(time.Now().Add(48 * time.Hour))),
			TasklistIDs: []int64{resourceIDs.tasklistID},
			TagIDs:      []int64{resourceIDs.tagID},
			Assignees: &twapi.LegacyUserGroups{
				UserIDs: []int64{resourceIDs.userID},
