	...
	mcpServer.AddTool(
		mcp.NewTool("create-project",
			...
			mcp.WithString("start-at",
				mcp.Description("The start date of the project in ISO 8601 format (e.g., 2023-01-01T00:00:00Z)."),
			),
			mcp.WithString("end-at",
				mcp.Description("The end date of the project in ISO 8601 format (e.g., 2023-12-31T23:59:59Z)."),
			),
			mcp.WithString("category-id",
				mcp.Description("The ID of the category to which the project belongs."),
			),
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			err := twmcp.ParamGroup(request.Params.Arguments,
				...
				twmcp.OptionalTimePointerParam(&project.StartAt, "start-at"),
				twmcp.OptionalTimePointerParam(&project.EndAt, "end-at"),
				twmcp.OptionalNumericPointerParam(&project.CategoryID, "category-id"),
				twmcp.RequiredNumericParam(&project.CompanyID, "company-id"),
